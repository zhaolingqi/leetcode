...
var findItinerary = function(tickets) {
  ...
  const dfs = (city, used) => { // 当前访问的城市city 已用掉的机票数used
    if (used == tickets.length) { // 遍历完所有“边”，返回true
      return true;
    };
    const nextCities = map[city];
    if (!nextCities || nextCities.length == 0) { // 没有邻接城市了
      return false; // 还没用光机票的情況下，就没有下一站了，返回false
    }
    for (let i = 0; i < nextCities.length; i++) { // 设置出多个选择（递归分支）
      const next = nextCities[i]; // 当前选择的下一站
      nextCities.splice(i, 1);    // 更新一下邻接表，飞出地的list中删掉这一站
